...
async function createTenant(input, tenantSlug, ownerEmail, ownerPhone) {
  ...
  const result = await run(
    `INSERT INTO tenants
     ...
     VALUES (?, ?, 'trial', 'trialing', ?, 'trialing', ?, ?, 'trial', ?, ?, ?, ?, NULL)`,
    [
...
async function createTenantOnboarding(input) {
  await validateOnboardingInput(input);
  ...
  return transaction(async () => {
    const tenant = await createTenant(input, tenantSlug, ownerEmail, ownerPhone);
    ...
    await run(
      `INSERT INTO tenant_subscriptions
       ...
       VALUES (?, ?, 'trial', 'trialing', 0, 'INR', CURRENT_TIMESTAMP, ?, CURRENT_TIMESTAMP, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL)`,
      [
        generateId('sub'),
        tenant.id,
        getFutureDate({ days: 14 }),
        getFutureDate({ days: 14 }),
      ]
    );
